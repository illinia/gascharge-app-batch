                    ...
                    publishers: [
                        sshPublisherDesc(
                            ...
                            transfers: [
                                ...
                                sshTransfer(
                                    execCommand: '/usr/local/bin/docker stop gascharge-app-batch',
                                    ignoreErrors: true
                                ),
                                sshTransfer(
                                    execCommand: '/usr/local/bin/docker rm gascharge-app-batch',
                                    ignoreErrors: true
                                ),
                                sshTransfer(
                                ...
                                sshTransfer(
                                    execCommand: 'nohup /usr/local/bin/docker run --name gascharge-app-batch -it -d -p 8400:8400 --privileged --cgroupns=host -v /sys/fs/cgroup:/sys/fs/cgroup:rw gascharge-app-batch /usr/sbin/init > nohup-app-batch-run.out 2>&1 &'
                                )
                            ]
                        )
                    ]
